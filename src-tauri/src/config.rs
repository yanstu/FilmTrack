...
impl WindowConfig {
    pub fn normalize(mut self) -> Self {
        const RECOMMENDED_WIDTH: u32 = 1280;
        const RECOMMENDED_HEIGHT: u32 = 800;
...
impl ConfigManager {
    ...
    pub fn save(app_handle: &AppHandle) -> Result<(), String> {
        // 串行化写入，避免并发覆盖/文件损坏
        let _guard = SAVE_LOCK.lock().unwrap_or_else(|poison| poison.into_inner());
        ...
        // 序列化配置
        let config = CONFIG.read().unwrap_or_else(|poison| poison.into_inner());
    ...
    pub fn get() -> AppConfig {
        CONFIG.read().unwrap_or_else(|poison| poison.into_inner()).clone()
    }
    
    /// 更新配置
    pub fn update(new_config: AppConfig, app_handle: &AppHandle) -> Result<(), String> {
        {
            let mut config = CONFIG.write().unwrap_or_else(|poison| poison.into_inner());
            *config = AppConfig {
    ...
    pub fn get_database_name() -> String {
        CONFIG.read().unwrap_or_else(|poison| poison.into_inner()).database.name.clone()
    }
    
    /// 获取缓存目录名称
    pub fn get_cache_dir_name() -> String {
        CONFIG.read().unwrap_or_else(|poison| poison.into_inner()).cache.dir_name.clone()
    }
    
    /// 获取请求间隔
    pub fn get_request_interval() -> u64 {
        CONFIG.read().unwrap_or_else(|poison| poison.into_inner()).scraper.request_interval_ms
    }
    
    /// 获取窗口配置
    pub fn get_window_config() -> WindowConfig {
        CONFIG.read().unwrap_or_else(|poison| poison.into_inner()).window.clone()
    }
    ...
    /// 仅更新窗口大小（在单次写锁内只修改尺寸字段，避免与位置写入相互覆盖）
    pub fn update_window_size_only(width: u32, height: u32, app_handle: &AppHandle) -> Result<(), String> {
        {
            let mut config = CONFIG.write().unwrap_or_else(|poison| poison.into_inner());
            config.window.width = width;
            config.window.height = height;
            config.window = config.window.clone().normalize();
        }
        Self::save(app_handle)
    }

    /// 仅更新窗口位置（在单次写锁内只修改位置字段，避免与尺寸写入相互覆盖）
    pub fn update_window_position_only(x: i32, y: i32, app_handle: &AppHandle) -> Result<(), String> {
        {
            let mut config = CONFIG.write().unwrap_or_else(|poison| poison.into_inner());
