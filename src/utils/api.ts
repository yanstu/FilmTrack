...
// 创建TMDb API请求队列实例（间隔 + 并发都来自配置）
export const tmdbQueue = new RequestQueue(
  APP_CONFIG.tmdb.request.interval,
  APP_CONFIG.tmdb.request.maxConcurrent ?? 2
);

// 429 / 限流自动退避重试：被限流时按 Retry-After 或指数退避稍等再试，
// 重试期间仍占用队列并发槽，从而自然把整体速率压回安全区，避免雪崩式 429。
...
function getRetryAfterMs(error: unknown, attempt: number): number {
  const header = (error as { response?: { headers?: Record<string, unknown> } } | undefined)
    ?.response?.headers?.['retry-after'];
  const retryAfterSeconds = Number(header);
  if (Number.isFinite(retryAfterSeconds) && retryAfterSeconds > 0) {
    return Math.min(retryAfterSeconds * 1000, 10000);
  }
  // 指数退避：250ms, 500ms, 1000ms（上限 2s）
  return Math.min(2000, 250 * 2 ** attempt);
}

async function requestWithRateLimitRetry<T>(fn: () => Promise<T>): Promise<T> {
  let attempt = 0;
  for (;;) {
    try {
      return await fn();
    } catch (error) {
      if (getErrorStatus(error) === 429 && attempt < RATE_LIMIT_MAX_RETRIES) {
        await delay(getRetryAfterMs(error, attempt));
        attempt += 1;
        continue;
      }
      throw error;
...
export const tmdbAPI = {
  ...
  async _request<T>(
    ...
    return tmdbQueue.add(cacheKey, async () => {
      try {
        const response = await requestWithRateLimitRetry(() => tmdbClient.get(endpoint, { params }));
  ...
  async searchMovies(
    ...
    // 并行试所有变体，再按原优先级取最靠前的非空结果（此前逐个串行 await，慢）
    const settled = await Promise.all(
      uniqueStrategies.map((searchKeyword) =>
        this.searchMoviesExact(searchKeyword, page).catch(() => null)
      )
    );
  ...
  async loadBackdropImages(
    ...
    return tmdbQueue.add(cacheKey, async () => {
      try {
        ...
        const response = await requestWithRateLimitRetry(() => tmdbClient.get(endpoint, {
          params: {
