<template>
  <div class="card p-6">
    ...
    <div class="space-y-3">
      <!-- 主操作：一体式快速记录（进度 + 评分 + 短评 一气呵成） -->
      <button
        @click="$emit('quickRecord')"
        class="w-full flex items-center justify-center gap-2 px-4 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors duration-200 font-medium shadow-sm"
      >
      ...
      <button
        ...
        class="w-full flex items-center justify-center px-4 py-3 border border-gray-200 text-gray-700 rounded-xl hover:bg-gray-50 transition-colors duration-200 font-medium"
      >
        编辑记录
      </button>
