    ...
    <div v-if="!hasImportSession">
      <h2 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
        ...
        <div class="mb-6">
          ...
          <div class="flex gap-3">
            ...
            <button
              ...
              class="inline-flex flex-shrink-0 items-center justify-center rounded-2xl border border-transparent bg-blue-600 py-3 px-5 text-sm font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors duration-200"
            >
        ...
        <p class="mt-2 text-sm text-gray-500 flex items-center">
          <InfoIcon class="w-4 h-4 mr-1 flex-shrink-0 text-gray-400" />
          在豆瓣个人主页链接里就能找到：douban.com/people/<strong>203503302</strong>/
        </p>
      </div>
      ...
      <div class="bg-blue-50/60 p-4 rounded-xl border border-blue-100/50 hover:bg-blue-50/80 transition-colors duration-300">
        ...
        <ul class="text-xs text-blue-700 list-disc pl-5 space-y-1.5">
          <li class="hover:text-blue-800 transition-colors duration-200">先确认豆瓣「看过」列表是公开的，否则读取不到记录</li>
          <li class="hover:text-blue-800 transition-colors duration-200">导入时先别关闭应用；切出去再回来，进度照常</li>
          <li class="hover:text-blue-800 transition-colors duration-200">会自动到 TMDb 补齐海报、简介和季集信息</li>
          <li class="hover:text-blue-800 transition-colors duration-200">你的评分、观看日期、季数和短评都会保留</li>
        </ul>
      </div>
    </div>
        ...
        <div class="mb-3 flex items-start justify-between gap-4">
          <div>
            ...
            <p class="mt-1 text-xs leading-5 text-amber-700">
              这里汇总了跳过、失败和低分匹配的条目，点开会自动带上搜索词去重新匹配。
            </p>
          </div>
...
const handleStartImport = () => {
  if (!doubanUserId.value.trim()) {
    appStore.modalService.showInfo('还差一步', '请先填写你的豆瓣用户 ID 再开始导入。');
    return;
  }
