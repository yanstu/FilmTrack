...
import { generateSearchKeywords, cleanTitle, calculateTitleSimilarity } from '../../../utils/titleUtils';
import { APP_CONFIG } from '../../../../config/app.config';
import type { ImportProgress, ImportLog, DoubanMovie } from '../../../types/import';
...
function extractSeasonNumber(title: string): number | null {
  ...
  const englishSeasonMatch = title.match(/Season\s*(\d+)/i) || title.match(/S(\d+)/i);
 ...
 * @returns 是否存在，以及相关数据
 */
const checkExistingMedia = async (title: string, type: string, tmdbId?: number): Promise<{ exists: boolean, data?: any }> => {
  try {
    // 获取所有影视作品
    ...
    // 首先通过TMDb ID匹配（如果有）
    if (tmdbId) {
      const existingByTmdbId = movies.find(movie => 
    ...
    const existingByTitle = movies.find(movie => 
      movie.type === type && 
      (
...
export function useDoubanImport() {
  ...
  const resetImport = () => {
    ...
    // 生成新的会话ID
    importSessionId.value = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
  };
  ...
  const startDoubanImport = async () => {
    ...
    try {
      ...
      const movieCount = await invoke<number>('estimate_douban_movie_count', { userId: doubanUserId.value });
        ...
        for (const movie of movies) {
          ...
          try {
            ...
            if (movie.type_ === 'movie' && existingMedia.exists) {
              await addLog('skip', `跳过电影 "${movie.title}" - 已存在于数据库`);
              importProgress.value.skipped++;
              continue;
            }
            
            // 如果是电视剧且已存在
            if (movie.type_ === 'tv' && existingMedia.exists && existingMedia.data) {
              const existingTVShow = existingMedia.data;
              
              // 如果有季数信息，只更新季数（不更新评分和其他信息）
              if (seasonNumber && seasonNumber > (existingTVShow.current_season || 0)) {
                addLog('info', `更新电视剧"${existingTVShow.title}"的季数: ${existingTVShow.current_season || 0} -> ${seasonNumber}`);
                ...
                addLog('success', `成功更新: "${existingTVShow.title}" 的季数为 ${seasonNumber}`);
              } else {
                addLog('skip', `跳过电视剧 "${movie.title}" - 已存在于数据库${seasonNumber ? `，且现有季数(${existingTVShow.current_season || 0})已大于或等于导入季数(${seasonNumber})` : ''}`);
                importProgress.value.skipped++;
              }
            ...
            if (APP_CONFIG.tmdb.request.enableTitleMatching) {
              // 根据类型搜索
              const searchMethod = movie.type_ === 'movie' 
                ? tmdbAPI.searchMovies.bind(tmdbAPI) 
                : tmdbAPI.searchTVShows.bind(tmdbAPI);
              
              try {
                await addLog('info', `TMDb搜索: "${movie.title}" -> 搜索标题: "${searchTitle}"`);
                ...
                if (searchResult.success && searchResult.data && searchResult.data.results && searchResult.data.results.length > 0) {
                  // 获取详细信息
                  const detailMethod = movie.type_ === 'movie' 
                    ? tmdbAPI.getMovieDetails.bind(tmdbAPI) 
                    : tmdbAPI.getTVDetails.bind(tmdbAPI);
                  
                  const detailResult = await detailMethod(searchResult.data.results[0].id);
                  
                  if (detailResult.success && detailResult.data) {
                    tmdbData = detailResult.data;
                    
                    // 再次检查是否存在相同的TMDb ID
                    const existingByTmdbId = await checkExistingMedia(movie.title, movie.type_ === 'movie' ? 'movie' : 'tv', tmdbData.id);
                    if (existingByTmdbId.exists && existingByTmdbId.data) {
                      // 如果是电影，直接跳过
                      if (movie.type_ === 'movie') {
                        addLog('skip', `跳过电影 "${movie.title}" - 已存在相同TMDb ID的电影: "${existingByTmdbId.data.title}"`);
                        importProgress.value.skipped++;
                        continue;
                      }
                      ...
                      if (movie.type_ === 'tv') {
                        ...
                        if (seasonNumber && seasonNumber > (existingTVShow.current_season || 0)) {
                          ...
                          addLog('skip', `跳过电视剧 "${movie.title}" - 已存在相同TMDb ID的电视剧: "${existingTVShow.title}"`);
                          importProgress.value.skipped++;
                        }
                    ...
                    addLog('success', `成功匹配TMDb数据: "${movie.title}" -> "${tmdbData.title || tmdbData.name}"`);
                  }
                }
              } catch (error) {
                console.error('TMDb搜索失败:', error);
                await addLog('warning', `TMDb搜索失败: "${movie.title}" (搜索标题: "${searchTitle}") - ${error}`);
              }
            }
            ...
            if (tmdbMatched && tmdbData) {
              ...
              movieData = {
                ...
                original_title: tmdbData.original_title || tmdbData.original_name || (tmdbData.title || tmdbData.name),
                overview: tmdbData.overview,
                poster_path: tmdbData.poster_path,
                backdrop_path: tmdbData.backdrop_path,
                tmdb_id: tmdbData.id,
                tmdb_rating: tmdbData.vote_average,
                year: new Date(tmdbData.release_date || tmdbData.first_air_date || '').getFullYear() || null,
                runtime: tmdbData.runtime || (tmdbData.episode_run_time ? tmdbData.episode_run_time[0] : null),
                genres: tmdbData.genres ? tmdbData.genres.map((g: any) => g.name).join(',') : '',
              ...
              if (movie.type_ === 'tv' && tmdbData.number_of_seasons) {
                ...
                if (!movieData.current_season && tmdbData.number_of_seasons === 1) {
                  movieData.current_season = 1;
                }
                
                // 已完成的剧集，设置当前集数为最后一集
                if (movieData.status === 'completed' && tmdbData.number_of_episodes) {
                  movieData.current_episode = tmdbData.number_of_episodes;
                }

                // 如果没有设置当前集数但状态是已完成，设置为最后一集
                if (movieData.status === 'completed' && !movieData.current_episode && tmdbData.number_of_episodes) {
                  movieData.current_episode = tmdbData.number_of_episodes;
                }
              }
            } else {
              ...
              await addLog('error', `未找到TMDb匹配: "${movie.title}" (搜索标题: "${searchTitle}"), 跳过导入`);
            ...
            if (result.success) {
              ...
              importProgress.value.failed++;
              await addLog('error', `导入失败: "${movieData.title}" - ${result.error}`);
            }
          } catch (error) {
            importProgress.value.failed++;
      ...
      if (shouldStop.value) {
        ...
        addLog('success', `导入完成! 成功: ${importProgress.value.success}, 跳过: ${importProgress.value.skipped}, 失败: ${importProgress.value.failed}`);
      }
    } catch (error) {
