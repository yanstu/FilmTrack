...
const MOVIES_TABLE_SQL = `CREATE TABLE IF NOT EXISTS movies (
  ...
  watched_date TEXT,
  notes TEXT,
  play_profile_id TEXT,
  play_profile_name TEXT,
  play_source_key TEXT,
  play_source_name TEXT,
  play_vod_id TEXT,
  play_vod_name TEXT,
  play_group_name TEXT,
  play_episode_name TEXT,
  play_episode_url TEXT,
  play_quality_label TEXT,
  play_selection_mode TEXT CHECK (play_selection_mode IN ('auto', 'manual')),
  play_match_score REAL,
  play_playback_rate REAL DEFAULT 1.0,
  play_last_played_at TEXT,
...
const MOVIE_PLAYBACK_PROGRESS_TABLE_SQL = `CREATE TABLE IF NOT EXISTS movie_playback_progress (
  id TEXT PRIMARY KEY,
  movie_id TEXT NOT NULL,
  profile_id TEXT,
  source_key TEXT NOT NULL,
  vod_id TEXT,
  play_group_name TEXT NOT NULL,
...
const MOVIES_COLUMN_MIGRATIONS = [
  ...
  'ALTER TABLE movies ADD COLUMN play_profile_id TEXT',
  'ALTER TABLE movies ADD COLUMN play_profile_name TEXT',
  'ALTER TABLE movies ADD COLUMN play_source_key TEXT',
  'ALTER TABLE movies ADD COLUMN play_source_name TEXT',
  'ALTER TABLE movies ADD COLUMN play_vod_id TEXT',
  'ALTER TABLE movies ADD COLUMN play_vod_name TEXT',
  'ALTER TABLE movies ADD COLUMN play_group_name TEXT',
  'ALTER TABLE movies ADD COLUMN play_episode_name TEXT',
  'ALTER TABLE movies ADD COLUMN play_episode_url TEXT',
  'ALTER TABLE movies ADD COLUMN play_quality_label TEXT',
  'ALTER TABLE movies ADD COLUMN play_selection_mode TEXT',
  'ALTER TABLE movies ADD COLUMN play_match_score REAL',
  'ALTER TABLE movies ADD COLUMN play_playback_rate REAL',
  'ALTER TABLE movies ADD COLUMN play_last_played_at TEXT',
...
const MOVIE_PLAYBACK_PROGRESS_COLUMN_MIGRATIONS = [
  'ALTER TABLE movie_playback_progress ADD COLUMN profile_id TEXT',
  'ALTER TABLE movie_playback_progress ADD COLUMN source_key TEXT',
  'ALTER TABLE movie_playback_progress ADD COLUMN vod_id TEXT',
  'ALTER TABLE movie_playback_progress ADD COLUMN play_group_name TEXT',
  'ALTER TABLE movie_playback_progress ADD COLUMN episode_name TEXT',
  'ALTER TABLE movie_playback_progress ADD COLUMN episode_url TEXT',
  'ALTER TABLE movie_playback_progress ADD COLUMN position_seconds REAL',
  'ALTER TABLE movie_playback_progress ADD COLUMN duration_seconds REAL',
  'ALTER TABLE movie_playback_progress ADD COLUMN playback_rate REAL',
  'ALTER TABLE movie_playback_progress ADD COLUMN completed INTEGER',
  'ALTER TABLE movie_playback_progress ADD COLUMN last_played_at TEXT',
  'ALTER TABLE movie_playback_progress ADD COLUMN created_at TEXT',
  'ALTER TABLE movie_playback_progress ADD COLUMN updated_at TEXT'
] as const
...
const INDEX_COMMANDS = [
  ...
  'CREATE INDEX IF NOT EXISTS idx_movies_play_source_key ON movies(play_source_key)',
  ...
  'CREATE INDEX IF NOT EXISTS idx_movie_playback_progress_movie_id ON movie_playback_progress(movie_id)',
  'CREATE UNIQUE INDEX IF NOT EXISTS idx_movie_playback_progress_episode_unique ON movie_playback_progress(movie_id, source_key, episode_url)',
  'CREATE INDEX IF NOT EXISTS idx_movie_playback_progress_last_played_at ON movie_playback_progress(last_played_at)'
] as const
...
export class DatabaseSchema {
  ...
  private static migrations: Migration[] = [
    ...
    {
      version: 3,
      name: 'movie-playback-support',
      up: async (db) => {
        await this.executeIgnoreDuplicate(db, MOVIE_PLAYBACK_PROGRESS_TABLE_SQL)
        await this.ensurePlaybackProgressColumns(db)

        await db.execute(`
          UPDATE movies SET
            play_playback_rate = COALESCE(play_playback_rate, 1.0)
          WHERE play_playback_rate IS NULL
        `)
        ...
        await db.execute(`
          UPDATE movie_playback_progress SET
            ...
            playback_rate = COALESCE(playback_rate, 1.0),
            completed = COALESCE(completed, 0),
            updated_at = COALESCE(updated_at, last_played_at, created_at, ?),
            created_at = COALESCE(created_at, last_played_at, ?),
            last_played_at = COALESCE(last_played_at, updated_at, created_at, ?)
          WHERE position_seconds IS NULL
            OR duration_seconds IS NULL
            OR playback_rate IS NULL
  ...
  private static async ensureBaseTables(
    ...
    await db.execute(MOVIE_PLAYBACK_PROGRESS_TABLE_SQL)
  }
  ...
  private static async ensurePlaybackProgressColumns(
    db: Awaited<ReturnType<typeof DatabaseConnection.getRawInstance>>
  ): Promise<void> {
    for (const command of MOVIE_PLAYBACK_PROGRESS_COLUMN_MIGRATIONS) {
      await this.executeIgnoreDuplicate(db, command)
    }
  }
