<template>
  <div ref="rootRef" class="date-field-shell">
    ...
    <Teleport to="body">
      ...
      >
        <div v-if="isOpen" ref="panelRef" class="date-panel" :style="panelStyle">
...
// 日历面板 Teleport 到 body，定位由 JS 计算，避免被弹窗 overflow 裁切
...
const updatePanelPosition = () => {
  ...
  const gap = 8;
  const pad = 12;
  const panelHeight = 360; // 日历面板大致高度
  const spaceBelow = viewportHeight - rect.bottom;
  const spaceAbove = rect.top;
  // 下方空间不足且上方更宽时向上弹出
  const above = spaceBelow < panelHeight + gap + pad && spaceAbove > spaceBelow;
  const width = Math.min(320, viewportWidth - pad * 2);
  const left = Math.min(
  ...
  panelStyle.value = {
    ...
    bottom: above ? `${viewportHeight - rect.top + gap}px` : 'auto'
  };
};
...
const handleDocumentPointerDown = (event: MouseEvent) => {
  ...
  // 日历已 Teleport 到 body，需同时排除面板自身，否则点日历会被当作「点外面」而关闭
...
.date-panel {
  /* 定位（fixed + left/top/bottom/transform-origin）由 JS 计算后通过内联 style 设置，
     面板 Teleport 到 body，避免被弹窗 overflow 裁切 */
